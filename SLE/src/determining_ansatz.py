...
def main(n_search: int = 30, sleep_every=20, sleep_time=120, log_csv=Path(__file__).resolve().parent/"media/ansatz_log.csv"): 
    """
    ...
    Args: 
        ...
        * sleep_every: int 
            * every # steps that computer sleeps to throttle cpu 
        * sleep_time: int 
            * how long computer sleeps per break to throttle cpu
        * log_csv: Path 
            * where to log every sweep result
    """
    ...
    with open(log_csv, mode="w", newline="") as csvfile:
        ...
        writer.writerow(["param", "iter", "value", "cost", "delta_cost", "better"])
