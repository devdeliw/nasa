...
def _convert_dict_to_theta(parameters: dict[str, float]) -> np.ndarray: 
    ...
    for name in PARAMETER_ORDER:
        if name in parameters.keys(): 
            parameter_vector.append(parameters[name])
    return np.asarray(parameter_vector) 
...
def plot_singlet_spectra(
    ...
    ax.plot(
        B, dI, lw=2, 
        label=f"d(singlet_pop)/dB - {'lock-in' if modulate else 'raw'}"
    )
    ...
    ax.set_title(
        f"Singlet Derivative Sweep -- {n_points} steps", 
        fontsize=14
    )
    ax.legend()
...
if __name__ == "__main__": 
    plot_singlet_spectra( 
        ...
        bmax=+85, 
        n_points=100, 
        parameters = load_params(Path(__file__).parent / "utils/params.yaml"), 
        n_jobs=2, 
        blocks_per_core=8, 
        modulate=True, 
        outdir=Path("../media/"), 
